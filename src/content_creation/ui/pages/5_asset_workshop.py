    ...
    if manifest:
        ...
        with tab_carousel:
            if carousel:
                ...
                st.markdown("**📋 Copy — Carousel Slides**")
                carousel_text = ""
                for slide in carousel.slides:
                    carousel_text += f"Slide {slide.slide_number}: {slide.title}\n{slide.body}\n(Visual: {slide.visual_note})\n\n"
                carousel_text += f"CTA Slide: {carousel.cta_slide}"
                st.code(carousel_text, language=None)
        ...
        with tab_newsletter:
            if newsletter:
                ...
                newsletter_body = ""
                for sect in newsletter.sections:
                    sect_title = sect.section_name.replace("_", " ").title()
                    newsletter_body += f"--- {sect_title} ---\n{sect.content}\n\n"
                newsletter_body += f"CTA:\n{newsletter.cta}"
                st.code(newsletter_body, language=None)
