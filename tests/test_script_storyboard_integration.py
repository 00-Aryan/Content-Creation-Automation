...
def script_registry(tmp_path):
    ...
    (tmp_path / "prompts" / "short_video.md").write_text("Test script prompt {{ brief.topic_id }}")
...
class TestScriptLegacyMode:
    ...
    def test_legacy_mode_no_storyboard(self, brief, valid_script_response, script_registry):
...
class TestScriptStoryboardMode:
    ...
    def test_storyboard_overrides_fields_short_video(self, brief, storyboard, valid_script_response, script_registry):
        with patch("content_creation.generation.script.InferenceManager") as mock_cls:
            mock_mgr = MagicMock()
            mock_cls.return_value = mock_mgr
            mock_mgr.generate.return_value = _make_result(valid_script_response)
    ...
    def test_storyboard_fallback_uses_storyboard_values(self, brief, storyboard, script_registry):
        with patch("content_creation.generation.script.InferenceManager") as mock_cls:
            mock_mgr = MagicMock()
            mock_cls.return_value = mock_mgr
            mock_mgr.generate.return_value = _make_result("", success=False)
        ...
        assert script.cta == "Try writing attention mechanisms in PyTorch"
        assert script.claims_used == ["Script claim 1", "Script claim 2"]
        assert script.script_sections == ["needs_review", "needs_review", "needs_review", "needs_review"]
...
class TestScriptMigration:
    ...
    def test_prompt_field_mapping(self, brief, storyboard, valid_script_response, tmp_path):
        ...
        (prompt_dir / "short_video.md").write_text("Metaphor: {{ brief.analogy }}\nClaims:\n{{ brief.plain_english_summary }}")
